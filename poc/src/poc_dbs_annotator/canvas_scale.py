"""Drag-enabled scale slider built on ``toga.Canvas``.

This replaces the Qt ``ScaleProgressWidget`` (see
``src/dbs_annotator/ui/widgets.py`` in the main app) which uses a QProgressBar
with an eventFilter to turn clicks/drags into value changes.

The widget exposes a simple ``on_change`` callback and a ``value`` property.
Values are clamped to the configured range and rendered as a filled bar with
the numeric value overlaid.
"""
