"""Production Toga port of ``ElectrodeCanvas``.

This is the hardened version of the PoC widget in
``poc/src/poc_dbs_annotator/canvas_electrode.py`` wired into the main
app's model classes (``config_electrode_models``). See that file for the
full architectural notes; the docstring here stays focused on the
production-specific hooks.

Behavioural parity with the Qt ``ElectrodeCanvas`` in
``src/dbs_annotator/models/electrode_viewer.py``:

* Same contact-state cycle (OFF -> ANODIC -> CATHODIC -> OFF) on tap.
* Same ring-level bulk toggle on the ring cap.
* Same validation hook -- caller receives ``(ok, message)`` whenever the
  configuration changes.
* ``to_png(path)`` matches the Qt ``_render_electrode_png`` output (used
  by ``utils/session_exporter.py``) -- the PNG has a white background so
  it embeds cleanly in Word reports.

The Qt backend exposes ``QRadialGradient`` highlights for a metallic 3D
look; Toga's iOS/Android canvas backends do not currently implement
radial gradients, so we use flat shading with a linear highlight strip
as documented in the migration plan (section 3).
"""
...
def _state_colors(state: int, hovered: bool):
...
def _point_in_polygon(px: float, py: float, poly: list[tuple[float, float]]) -> bool:
    """Ray-cast polygon containment test; works on every Toga backend."""
    inside = False
    n = len(poly)
    j = n - 1
    for i in range(n):
...
def _point_in_rect(
    px: float, py: float, rect: tuple[float, float, float, float], pad: float = 0.0
) -> bool:
    x, y, w, h = rect
...
class ElectrodeCanvas(toga.Box):
    ...
    def set_state(
        self,
        contact_states: dict[tuple[int, int], int],
        case_state: int = ContactState.OFF,
    ) -> None:
        self.contact_states = dict(contact_states)
    ...
    def _notify_validation(self) -> None:
        if self.on_validation is None:
            return
        ok, msg = StimulationRule.validate_configuration(
            self.contact_states, self.case_state
        )
    ...
    def _repaint(self) -> None:
        ...
        with ctx.Fill(color=case_text_color) as label_fill:
            label_fill.write_text(
                ...
                case_y + case_h / 2 + 4,
                font=toga.Font(family="sans-serif", size=max(7, int(scale * 0.3))),
            )
